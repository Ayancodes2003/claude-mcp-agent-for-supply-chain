"""
Interface to the Claude API for the warehouse simulation.
"""
import os
import json
import logging
...
class ClaudeInterface:
    ...
    def ask(
        ...
        try:
            ...
            # Add warehouse state to the system prompt if provided
            if warehouse_state:
                state_json = json.dumps(warehouse_state, indent=2)
                system_prompt += f"\n\nCurrent warehouse state:\n```json\n{state_json}\n```"
            
            # Log the request
            self.logger.info(f"Sending request to Claude ({self.model})")
            ...
            # Log the response
            self.logger.info(f"Received response from Claude ({len(content)} chars)")
            ...
            self.logger.error(f"Error querying Claude: {str(e)}")
            return {
                "prompt": prompt,
                "error": str(e),
                "timestamp": datetime.now().isoformat()
